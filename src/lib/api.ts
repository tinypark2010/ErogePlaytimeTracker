...
export const api={
 ...
 createGame:(input:{title:string;brand?:string;release_date?:string;thumbnail_url?:string;erogamescape_id?:number;source_url?:string;executable_paths:string[]})=>invoke<number>('create_game',{input}),
 updateGame:(id:number,input:{title:string;brand?:string;release_date?:string;source_url?:string})=>invoke<void>('update_game',{id,input}), openExternalUrl:(url:string)=>invoke<void>('open_external_url',{url}), deleteGame:(id:number)=>invoke<void>('delete_game',{id}),
 addExecutable:(gameId:number,path:string)=>invoke<void>('add_game_executable',{gameId,path}), removeExecutable:(id:number)=>invoke<void>('remove_game_executable',{id}),
 launchGame:(gameId:number)=>invoke<void>('launch_game',{gameId}),
 sessions:(gameId:number)=>invoke<Session[]>('list_sessions',{gameId}), intervals:(sessionId:number)=>invoke<FocusInterval[]>('list_focus_intervals',{sessionId}),
 manualSession:(gameId:number,start:string,end:string)=>invoke<number>('create_manual_session',{gameId,start,end}),
 updateSession:(id:number,start:string,end:string|null)=>invoke<void>('update_session',{id,start,end}), deleteSession:(id:number)=>invoke<void>('delete_session',{id}),
