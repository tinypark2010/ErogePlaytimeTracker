export type SortKey='title'|'brand'|'release_date'|'created_at'|'total_playtime'|'last_played'|'session_count';
export interface GameSummary {id:number;title:string;brand:string|null;release_date:string|null;thumbnail_path:string|null;created_at:string;total_playtime_seconds:number;total_running_seconds:number;last_played:string|null;session_count:number}
export interface Executable {id:number;game_id:number;path:string;file_name:string;created_at:string}
...
export interface TrackingStatus {running_games:Array<{game_id:number;title:string;session_id:number}>;foreground_game_id:number|null}
