<script lang="ts">
 ...
 let page:'library'|'game'|'add'|'settings'='library', gameId=0, refresh=0, status:TrackingStatus={running_games:[],foreground_game_id:null};
 ...
 onMount(()=>{api.settings().then(v=>applyTheme(v.theme));api.status().then(v=>status=v);const timer=setInterval(()=>api.status().then(v=>status=v),3000);let off=()=>{};listen<TrackingStatus>('tracking-status',e=>status=e.payload).then(f=>off=f);return()=>{clearInterval(timer);off()}});
</script>
