<script lang="ts">
  ...
  let page: 'library' | 'game' | 'add' | 'settings' = 'library',
    gameId = 0,
    refresh = 0,
    status: TrackingStatus = { running_games: [], foreground_game_id: null },
    pendingBackground: TrackingStatus | null = null,
    backgroundTimer: number | undefined;
  ...
  $: trackingText = !isRunning
    ? '待機中'
    : isForeground
      ? `プレイ時間を記録中（${status.running_games.find((g) => g.game_id === status.foreground_game_id)?.title ?? 'ゲーム'}）`
      : `${status.running_games.length}本起動中・バックグラウンド時間は除外中`;
  const applyTheme = (theme: Theme) => (document.documentElement.dataset.theme = theme);
  function updateStatus(next: TrackingStatus) {
    const background = next.running_games.length > 0 && next.foreground_game_id === null;
    const alreadyBackground = status.running_games.length > 0 && status.foreground_game_id === null;
    if (background && !alreadyBackground) {
      pendingBackground = next;
      if (backgroundTimer === undefined)
        backgroundTimer = window.setTimeout(() => {
          if (pendingBackground) status = pendingBackground;
...
<header>
  ...
  <div
    ...
    class:active={isForeground}
    class:background={isRunning && !isForeground}
  >
    ● {trackingText}
  </div>
</header>
