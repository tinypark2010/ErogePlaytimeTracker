...
ゲーム画面からのOCR検索は`hotkeys.rs`で撮影キーとは別に登録し、`game_ocr.rs`の専用threadでforegroundの計測中gameを確認してclient areaを取得します。取得したframeはmemory上だけで保持し、Win32の枠なしoverlayで範囲を選択して同じOCR engineへ渡します。物理pixel単位の座標でposition・selection・cropを揃え、画像fileやDB recordは作りません。選択完了を検索操作として扱い、空でない認識結果だけを既定browserのGoogle検索へ送ります。Esc・右クリック・focus lossで中止でき、処理中の再起動は抑止します。overlayやbrowserへのfocus移動は通常どおりbackground時間として計測します。追加のWebView、asset scope、Tauri capabilityは不要です。
