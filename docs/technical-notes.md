...
起動直後など、関連processはあるがvisible windowがまだない期間はBackgroundとして扱いません。gameのvisible windowが消えた後も一時的なBackground recordを作らず、最後にwindowが消えたtimestampでsessionを閉じます。異常終了後はperiodic `last_seen`より後を加算せず、orphan recordを閉じて`needs_review`にします。
