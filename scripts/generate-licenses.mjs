...
function licenseText(packageDir) {
  ...
  const file = readdirSync(packageDir)
    .sort()
...
const licenseSections = [...groups.values()].map((group, index) => {
  ...
  const text =
    group.text ??
    'The package did not include a standalone license file. Its declared SPDX license expression is listed above; consult the package repository for the complete terms.';
...
writeFileSync(
  ...
  header + inventory + divider + 'LICENSE TEXTS' + divider + licenseSections.join(divider) + '\n',
);
console.log(
  `Generated THIRD_PARTY_LICENSES.txt with ${dependencies.length} dependencies and ${groups.size} unique license texts.`,
);
